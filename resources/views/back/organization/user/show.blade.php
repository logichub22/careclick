					...
					<div class="col-md-12">
						@if($user->status)
					 		...
					 		{!! Form::open(['route' => ['deactivate', $user->id], 'method' => 'POST', 'style' => 'display: inline-block', 'id' => 'deactivateForm']) !!}
                               ...
                               {{Form::button('Deactivate User', array('id'=> 'deactivate', 'type' => 'submit', 'class' => 'btn btn-block btn-danger', 'title' => 'Deactivate User'))}}
                            {!! Form::close() !!}
						@else
							{!! Form::open(['route' => ['activate', $user->id], 'method' => 'POST', 'style' => 'display: inline-block', 'id' => 'activateForm']) !!}
							   <input type="hidden" name="user_id" value="{{ $user->id }}">
					 		   <input type="hidden" value="{{ $user->name }}" name="name" id="name">
							   <input type="hidden" value="{{ $user->email }}" name="email">
                               {{Form::button('Activate User', array('id'=> 'activate', 'type' => 'submit', 'class' => 'btn btn-block btn-success', 'title' => 'Activate User'))}}
                            {!! Form::close() !!}
						@endif
					</div>
				</div>
