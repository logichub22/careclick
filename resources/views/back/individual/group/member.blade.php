				...
				<div class="row">
					<div class="col-sm-12 col-md-6">
						@if($user->memberstatus)
					 		...
					 		{!! Form::open(['route' => ['user.cancelmember', $user->memberid],'method' => 'POST', 'style' => 'display: inline-block', 'id' => 'cancelForm']) !!}
                            ...
                            {!! Form::close() !!}
						@else
							{!! Form::open(['route' => ['user.renewmember', $user->memberid],'method' => 'POST', 'style' => 'display: inline-block', 'id' => 'renewForm']) !!}
							   <input type="hidden" name="group" value="{{ $group->name }}">
					 		   <input type="hidden" value="{{ $user->name }}" name="member_name">
							   <input type="hidden" value="{{ $user->email }}" name="email">
					...
					<div class="col-sm-12 col-md-6">
						{!! Form::open(['route' => ['user.deletemember', $user->memberid],'method' => 'POST', 'style' => 'display: inline-block', 'id' => 'deleteForm']) !!}
							<input type="hidden" name="group" value="{{ $group->name }}">
				 		    <input type="hidden" value="{{ $user->name }}" name="member_name">
						    <input type="hidden" value="{{ $user->email }}" name="email">
				...
				<div class="card-body">
					...
					<div class="tab-content">
						...
						<div class="tab-pane" id="edit" role="tabpanel" aria-labelledby="edit-tab">
							<br>
							<form action="{{ route('users.updatemember') }}" method="POST" enctype="multipart/form-data">
								{{ csrf_field() }}
						...
						<div class="tab-pane" id="message" role="tabpanel" aria-labelledby="message-tab">
							<br>
							<form action="{{ route('user.emailmember') }}" method="POST">
								{{ csrf_field() }}
