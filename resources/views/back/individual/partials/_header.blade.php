		...
		<div class="navbar-right navbar-toggleable-sm collapse" id="collapse-1">
			...
			<ul class="nav navbar-nav float-md-right">
				...
				<li class="nav-item dropdown hidden-sm-down">
					...
					<i class="fas fa-flag"></i>  @lang('individual.language')
					</a>
					<div class="dropdown-menu dropdown-menu-right animated fadeInUp">
						<a class="dropdown-item" href="{{ url('/change-language') }}?lang=en">
							<i class="fas fa-flag"></i> @lang('individual.english')
						</a>
						<a class="dropdown-item" href="{{ url('/change-language') }}?lang=sw">
							<i class="fas fa-flag"></i> @lang('individual.kiswahili')
						</a>
				...
				<li class="nav-item dropdown">
					<a class="nav-link" href="#" data-toggle="dropdown" aria-expanded="false">
						<i class="fas fa-bell {{ count(Auth::user()->unreadNotifications) > 0 ? 'bell' : '' }}"></i>
						<span class="hidden-md-up ml-1">@lang('individual.notifications')</span>
						<span class="tag tag-danger top">{{ count(Auth::user()->unreadNotifications) }}</span>
					</a>
					<div class="dropdown-messages dropdown-tasks dropdown-menu dropdown-menu-right">
						@foreach(Auth::user()->unreadNotifications as $notification)
							<div class="m-item">
								<div class="mi-icon bg-info"><i class="ti-comment"></i></div>
								<div class="mi-text"><a class="text-black" href="#">{{ $notification->data['data'] }}</a></div>
								<div class="mi-time">{{ \Carbon\Carbon::createFromTimeStamp(strtotime($notification->created_at))->diffForHumans() }}</div>
							</div>
						@endforeach
						<a class="dropdown-more" href="{{ route('user.unread') }}">
							<strong>@lang('individual.viewallnotifications')</strong>
						</a>
					</div>
				</li>
				<li class="nav-item dropdown hidden-sm-down">
					...
					<div class="dropdown-menu dropdown-menu-right animated fadeInUp">
						<a class="dropdown-item" href="{{ route('organization.profile') }}">
							<i class="ti-user mr-0-5"></i> @lang('individual.profile')
						</a>
						<a class="dropdown-item" href="{{ url('/') }}">
							<i class="fa fa-home mr-0-5"></i> @lang('individual.profile')
						</a>
						<div class="dropdown-divider"></div>
						<a class="dropdown-item" href="{{ route('logout') }}" onclick="event.preventDefault(); document.getElementById('logout-form').submit();"><i class="ti-power-off mr-0-5"></i> @lang('individual.signout')</a>
