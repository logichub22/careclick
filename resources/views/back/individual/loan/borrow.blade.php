		...
		<div class="col-md-4">
			<div class="card">
				...
				<div class="card-body">
					<div class="row">
						<div class="col-md-6">
							<a href="{{ route('user.browseloans') }}" class="btn btn-primary btn-block">Back to all packages</a>
						</div>
						<div class="col-md-6">
							<a href="{{ route('user-loans.index') }}" class="btn btn-success btn-block">My Loans</a>
						</div>
					</div>
				</div>
			</div>
	...
	<div class="row">
		<div class="col-md-12">
			<div class="card">
				...
				<div class="card-body">
					<form id="borrowForm" method="POST" action="{{ route('user-loans.store') }}">
					{{ csrf_field() }}
					...
					<input type="hidden" name="wallet_balance" value="{{ $bal }}" id="wallet_balance">
					...
					<div class="row">
						<div class="col-md-6">
							<div class="form-group">
								...
								<input type="text" class="form-control{{ $errors->has('loan_title') ? ' is-invalid' : '' }}" name="loan_title" value="{{ old('loan_title') }}" placeholder="How do you want to name your loan?" id="loan_title" required>
						...
						<div class="col-md-6">
							<div class="form-group">
								...
								<input type="text" class="form-control{{ $errors->has('amount') ? ' is-invalid' : '' }}" name="amount" value="{{ old('amount') }}" placeholder="Min: {{ number_format($package->min_amount) . ' ' . $package->currency . ' ' . ' ' }} Max: {{ number_format($package->max_amount) . ' ' . $package->currency }}" id="amount" onblur="restrictMinMaxAmount()" required>
								@if($errors->has('loan_title'))
									<span class="invalid-feedback" role="alert">
										{{ $errors->first('loan_title') }}
									</span>
							...
							<div class="form-group">
								<label for="length_of_loan">
									...
									@if($package->repayment_plan === "monthly")
										<select name="length_of_loan" class="form-control{{ $errors->has('length_of_loan') ? ' invalid-feddback' : '' }}" required>
									...
									@elseif($package->repayment_plan === "bi-weekly")									
										<input type="number" class="form-control{{ $errors->has('length_of_loan') ? ' is-invalid' : '' }}" name="length_of_loan" value="{{ old('length_of_loan') }}" placeholder="Length of loan in number" id="length_of_loan" min="1" max="26" required>
										...
										@endif
									@else							
										<input type="number" class="form-control{{ $errors->has('length_of_loan') ? ' is-invalid' : '' }}" name="length_of_loan" value="{{ old('length_of_loan') }}" placeholder="Length of loan in number" id="length_of_loan" min="1" max="52" required>
										@if($errors->has('length_of_loan'))
											<span class="invalid-feedback" role="alert">
												{{ $errors->first('length_of_loan') }}
											</span>
...
@section('spec-scripts')
	<script type="text/javascript">
		var max_amount = {!! json_encode($package->max_amount, JSON_HEX_TAG) !!};
		var min_amount = {!! json_encode($package->min_amount, JSON_HEX_TAG) !!};
